...
def query_hf_api(payload: dict) -> dict:
    ...
    if r.status_code != 200:
        try:
            ...
            msg = (r.text or "").strip() or r.reason or "Requests failed"
...
def _run_models(models, massage, max_tokens=160, temperature=0.3):
    last_err = None
    for model in models:
        data, err = query_hf_api({"model": model, "messages": massage, "max_tokens": max_tokens, "temperature": temperature})
        if err:
            last_err = err
            continue
...
def main():
    image_path = input(f"{Fore.CYAN}Enter the path to the image file: {Style.RESET_ALL}").strip()
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"{Fore.GREEN}Summary (50 words): {Fore.YELLOW}{Style.BRIGHT}{output}\n")
            except Exception as e:
            ...
            print(f"{Fore.RED}Invalid choice. Please select a valid option. Enter 1-4.\n")
