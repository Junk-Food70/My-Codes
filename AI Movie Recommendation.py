...
# Load and preproces the dataset
...
genres = list_genres(movie_df)

#Recommend movies based on filters (genre, mood, rating)
def recommend_movies(genre =None, moos=None, rating=None, top_n=5):
    filtered_df = recommend_movieses_df
    if genre:
...
def handle_ai(name):
    ...
    while True:
        ...
        if genre_input.indigit() and 1 <= int(genre_input) <= len(genres):
            genre = genres[int(gnere_input)-1]
            break
    ...
    mood_desc = "positive" # if polarity < 0 else "neutral"
    ...
    while True:
        ...
        if action == 'no':
            ...
            recs = recommendation_movies(genre=genre, mood=mood, rating=rating, top_n=5)
            if insistance(recs, str):
                print(Fore.RED + recs + "\n")
            else:
                display_recommendations(recs, name)
        else:
